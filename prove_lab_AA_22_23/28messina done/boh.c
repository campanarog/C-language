...
params readinput(int argc, char**argv){
    ...
    if(p.a> p.b){
        fprintf(stderr," a cant be greater than b\n");
        exit(EXIT_FAILURE);
    }
    return p;
}
...
char* dequeue(queue* q) {
    ...
    nodo* tmp = q->testa;
    char* str = strdup(tmp->data); // Usa strdup per allocare e copiare la stringa
    if (str == NULL) {
        fprintf(stderr, "Memory allocation failed\n");
        exit(EXIT_FAILURE);
    }
    ...
    free(tmp->data); // Libera la stringa originale
...
void buildqueue(queue*q, const char* input){
    ...
    while(fgets(buffer, sizeof(buffer), fp )!= NULL){
        buffer[strcspn(buffer, "\n")]='\0'; /* mettere sempre quando ci sono stringhe
        aiuta in maniera indiretta a non creare qualora ci fossero equivoci nel segmentation fault*/
        enqueue(q, buffer);
    }
    fclose(fp);
}
