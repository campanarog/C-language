...
struct parametri decodeParameters(int argc, char *argv[]) {
    ...
    /*char *extension = strrchr(p.filename, '.');  // Cerca l'ultimo punto nella stringa del filename
    if (!extension || strcmp(extension, ".txt") != 0) {  // Controlla se il punto esiste e se segue con "txt"
    fprintf(stderr, "Error: file must have .txt extension.\n");
    exit(EXIT_FAILURE);*/
